--
-- Migration of HR Schema
--  Create the user

create user TEST10 identified by 'HrAdmin';
GRANT ALL PRIVILEGES ON hr.* To TEST10; 
GRANT ALL PRIVILEGES ON ap.* To TEST10; 
GRANT ALL PRIVILEGES ON ex.* To TEST10; 
GRANT ALL PRIVILEGES ON om.* To TEST10; 

--  The next command does not work -  Still researching
--  connect --user=rolando -password=xxxyyy;
...
-- ***********trigger for adding whose salary is only greater than 0 *********
DELIMITER //
CREATE TRIGGER emp_sal_min BEFORE INSERT ON employees FOR EACH ROW 
BEGIN IF (new.salary <= 0 ) THEN call unknown(); 
END IF;
 END// DELIMITER ;
-- ********** adding constraints to employee table ********
...
-- ****** adding constraint in dept table*********
ALTER TABLE departments
...
DELIMITER // CREATE TRIGGER jobhist_dates BEFORE INSERT ON job_history FOR EACH ROW BEGIN IF (end_date > start_date ) THEN call unknown();
END IF;
END//
DELIMITER ;

-- **********Adding constraint on job history table ********** 
...
-- ******** Creating Emplyee detailsview to join with other tables
...
-- ********** INSERTING VALUES INTO - COUNTRIES *********
...
-- *********** INSERTING VALUES INTO - LOCATIONS ************
...
-- *********** DISABLING foreign key CONSTRAINT FROM DEPT **********
